...
def week_of(row: Row, using: str = "made") -> str:
    ...
    """
    year, week, _ = date_of(row, using).isocalendar()
    return f"{year:04d}-W{week:02d}"


# Grouping by name, for the command line to choose between. The keys are what
# `--by-week` and the default select, what the summary calls itself, and what
# `[periods]` in the rules file has to name a date for.
OF = {"month": month_of, "week": week_of}


def grouping(by: str, rules) -> "Callable[[Row], str]":
    """The function that puts a row in a period, with its date already chosen.

    Bound once per run rather than looked up per row, so that the decision about
    which date a grouping uses is made in one place and cannot drift between the
    summary and anything checking it.
    """
    if by not in OF:
        raise ValueError(f"cannot group by {by!r}, only by {' or '.join(sorted(OF))}")
    using = rules.date_for(by)
    return lambda row: OF[by](row, using)
