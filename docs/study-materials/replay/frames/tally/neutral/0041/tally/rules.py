...
# Which date a grouping is lined up on.
MADE = "made"
POSTED = "posted"
DATES = (MADE, POSTED)

# What may be listed in [duplicates] match.
MATCH_FIELDS = ("date", "posted", "amount", "description")

# What may be done with a description matching no rule.
STOP = "stop"
BUCKET = "bucket"
UNMATCHED = (STOP, BUCKET)
...
class Rules:
    ...
    @property
    def stops_on_unmatched(self) -> bool:
        return self.unmatched == STOP

    def date_for(self, by: str) -> str:
        """Which date the given grouping is lined up on."""
        return self.period_dates[by]
...
def parse(raw: str, path: Path = DEFAULT_PATH) -> Rules:
    ...
    words = _require(transfers, "words", list, "transfers", path)
    if not all(isinstance(word, str) for word in words):
        raise RulesError(f"{path}: [transfers] words should all be strings")

    handling = _require(transfers, "handling", str, "transfers", path)
    if handling not in HANDLING:
        raise RulesError(
            f"{path}: [transfers] handling should be one of "
            f"{', '.join(repr(name) for name in HANDLING)}, got {handling!r}"
        )

    # Imported here rather than at the top: periods imports nothing from this
    # module, and keeping it that way means the rules can be read without
    # dragging the policies in behind them.
    from .periods import OF

    period_table = data.get("periods", {})
    period_dates: dict[str, str] = {}
    for grouping in sorted(OF):
        chosen = _require(period_table, grouping, str, "periods", path)
        if chosen not in DATES:
            raise RulesError(
                f"{path}: [periods] {grouping} should be one of "
                f"{', '.join(repr(name) for name in DATES)}, got {chosen!r}"
            )
        period_dates[grouping] = chosen

    match = _require(data.get("duplicates", {}), "match", list, "duplicates", path)
    if not match:
        raise RulesError(
            f"{path}: [duplicates] match is empty, so every row would be a duplicate of the first"
        )
    unknown = [field for field in match if field not in MATCH_FIELDS]
    if unknown:
        raise RulesError(
            f"{path}: [duplicates] match has no field {unknown[0]!r}; "
            f"choose from {', '.join(MATCH_FIELDS)}"
        )

    unmatched = _require(data.get("categories", {}), "unmatched", str, "categories", path)
    if unmatched not in UNMATCHED:
        raise RulesError(
            f"{path}: [categories] unmatched should be one of "
            f"{', '.join(repr(name) for name in UNMATCHED)}, got {unmatched!r}"
        )
    ...
    return Rules(
        ...
        transfer_handling=handling,
        period_dates=period_dates,
        duplicate_match=tuple(match),
        unmatched=unmatched,
