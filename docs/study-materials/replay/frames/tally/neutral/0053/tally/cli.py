...
def main(argv: list[str] | None = None) -> int:
    ...
    if command == "check":
        ...
        if not found:
            ...
            return 2
        # Every statement is tried even after one of them refuses, because the
        # point of `check` is to run over a folder and be told everything that
        # is wrong with it in one go.
        refused = 0
        for path in found:
            try:
        ...
        if refused:
            print(f"{refused} of them would not summarise", file=sys.stderr)
            return 2
        return 0
