...
`tally/rules.toml` holds the merchant patterns, which date each grouping is
lined up on, what makes two rows a duplicate, what happens to an unknown
merchant, the wording that means a transfer and what to do with it, how many
months make a payment recurring, and the share of positive rows that means an
export has its signs the other way round. Adding a shop is an edit to a list.
