...
# Has transfers and a merchant no rule covers, so it needs the bucket.
STATEMENT = str(FIXTURES / "current.csv")

# Covered entirely by the shipped rules, so it runs under the strict default.
CLEAN = str(FIXTURES / "one-shop-three-ways.csv")
...
def test_a_summary_goes_to_stdout_when_asked(capsys):
    assert main(["summarise", CLEAN, "-"]) == 0
    assert "## 2026-01" in capsys.readouterr().out


def test_by_week_changes_the_grouping(capsys):
    assert main(["summarise", CLEAN, "-", "--by-week"]) == 0
    out = capsys.readouterr().out
    assert "## 2026-W02" in out and "## 2026-01\n" not in out
...
def test_the_option_can_go_before_the_file(capsys):
    ...
    assert main(["summarise", "--by-week", CLEAN, "-"]) == 0
    assert "## 2026-W02" in capsys.readouterr().out


def test_writing_goes_beside_the_statement(tmp_path, capsys):
    statement = tmp_path / "mine.csv"
    statement.write_text(Path(CLEAN).read_text(), encoding="utf-8")
...
def test_an_unknown_merchant_stops_the_run_and_names_it(capsys):
    assert main(["summarise", STATEMENT, "-"]) == 2
    captured = capsys.readouterr()
    assert "MOONLIGHT RECORDS" in captured.err
    assert not captured.out, "nothing half written"


def test_check_tries_every_statement_even_after_one_refuses(capsys):
    # The point of check is to be told everything wrong with a folder in one go.
    assert main(["check", str(FIXTURES)]) == 2
    captured = capsys.readouterr()
    assert "MOONLIGHT RECORDS" in captured.err
    assert "boundary.csv:" in captured.out, "the ones that work still report"
    assert "checked 4 statements" in captured.out


def test_the_bucket_can_be_asked_for_instead(tmp_path, capsys):
    mine = rules_file(tmp_path, categories='unmatched = "bucket"',
                      merchants="[[rule]]\npattern = 'tesco'\ncategory = \"groceries\"\n")
    assert main(["summarise", STATEMENT, "-", "--rules", str(mine)]) == 0
    assert "uncategorised" in capsys.readouterr().out
