...
def sign_convention(rows: list[Row], rules: Rules) -> list[Row]:
    ...
    guessed from the data: if more than `positive_share` of the rows are
    positive, this is one of the other kind, and every sign is flipped.
    ...
    if positive > len(rows) * rules.positive_share:
        for row in rows:
