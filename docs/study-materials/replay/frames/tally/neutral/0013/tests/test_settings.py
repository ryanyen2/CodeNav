...
def test_the_shipped_rules_load():
    ...
    assert loaded.unmatched == "bucket"
...
def test_which_date_decides_each_summary_comes_from_the_file(tmp_path):
    assert dict(rules(tmp_path).period_dates) == {"month": "made", "week": "posted"}
    swapped = rules(tmp_path, MINIMAL.replace('week = "posted"', 'week = "made"'))
    assert swapped.period_dates["week"] == "made"


def test_what_makes_two_rows_one_transaction_comes_from_the_file(tmp_path):
    assert dict(rules(tmp_path).duplicates) == {
        "month": "same wording", "week": "any wording",
    }


def test_a_summary_left_out_of_a_per_summary_table_is_an_error(tmp_path):
    # Not defaulted. The summary nobody wrote a line for would quietly answer a
    # different question from the one they did write.
    with pytest.raises(settings.RulesError) as raised:
        rules(tmp_path, MINIMAL.replace('week = "posted"', ""))
    assert "week" in str(raised.value)


def test_a_summary_that_does_not_exist_is_an_error(tmp_path):
    # Both named correctly, plus one that is not a summary at all. Somebody who
    # writes a line expecting it to do something should be told it does not.
    with pytest.raises(settings.RulesError) as raised:
        rules(tmp_path, MINIMAL.replace('week = "posted"',
                                        'week = "posted"\nfortnight = "made"'))
    assert "fortnight" in str(raised.value)


@pytest.mark.parametrize("bad", ['week = "cleared"', "week = 3"])
def test_a_date_that_does_not_exist_is_an_error(tmp_path, bad):
    with pytest.raises(settings.RulesError) as raised:
        rules(tmp_path, MINIMAL.replace('week = "posted"', bad))
    assert "made" in str(raised.value) and "posted" in str(raised.value)


def test_a_way_of_matching_that_does_not_exist_is_an_error(tmp_path):
    with pytest.raises(settings.RulesError) as raised:
        rules(tmp_path, MINIMAL.replace('week = "any wording"', 'week = "loose"'))
    assert "any wording" in str(raised.value)


@pytest.mark.parametrize("bad", ['unmatched = "carry on"', "unmatched = true", ""])
def test_a_way_of_handling_an_unknown_merchant_that_does_not_exist(tmp_path, bad):
    with pytest.raises(settings.RulesError) as raised:
        rules(tmp_path, MINIMAL.replace('unmatched = "bucket"', bad))
    assert "stop" in str(raised.value) and "bucket" in str(raised.value)


def test_what_happens_to_transfers_comes_from_the_file(tmp_path):
    assert rules(tmp_path).show_transfers == "apart"
    assert rules(tmp_path, MINIMAL.replace('"apart"', '"never"')).show_transfers == "never"


def test_what_transfers_are_called_comes_from_the_file(tmp_path):
    mine = rules(tmp_path, MINIMAL.replace('category = "transfers"', 'category = "savings"'))
    assert mine.transfer_category == "savings"
...
@pytest.mark.parametrize("bad", ['show = "apar"', 'show = "hide"', "show = false", ""])
def test_a_way_of_handling_transfers_that_does_not_exist_is_an_error(tmp_path, bad):
    # A misspelling would otherwise fall through to whichever branch runs last,
    # and the wrong one either hides money or counts money that was not spent.
    with pytest.raises(settings.RulesError) as raised:
        rules(tmp_path, MINIMAL.replace('show = "apart"', bad))
    assert "apart" in str(raised.value) and "never" in str(raised.value)
