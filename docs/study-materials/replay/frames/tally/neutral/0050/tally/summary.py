...
def summarise(raw: str, rules: Rules | None = None, by: str = "month") -> Summary:
    ...
    period_of = periods.grouping(by, rules)
    ...
    moved_rows = [row for row in kept if dedupe.is_transfer(row, rules)]
    result.transfers = len(moved_rows)

    # Counted even when they are hidden, because the one line a run prints has
    # always said how many there were, and that is the number that makes somebody
    # ask where they went.
    moved: dict[str, Decimal] = defaultdict(Decimal)
    moves: dict[str, int] = defaultdict(int)
    if rules.shows_transfers:
        for row in moved_rows:
            moved[period_of(row)] += row.amount
            moves[period_of(row)] += 1
    ...
    for row in spending:
        ...
        if name == categories.UNCATEGORISED:
            ...
            if row.description not in unknown:
                unknown.append(row.description)
        buckets[period_of(row)][name].append(row.amount)
        counts[period_of(row)] += 1

    # After the whole file, not during it, so one run names every merchant that
    # needs writing down instead of stopping at the first and being run again.
    if unknown and rules.stops_on_unmatched:
        raise categories.Unmatched(sorted(unknown))

    # A period that held nothing but a transfer still gets a section. It has no
    # spending to report and its total is zero, which is exactly the month the
    # reader is looking for when they ask where the money went.
    for name in sorted(set(buckets) | set(moved)):
        period = Period(
            name=name,
            transactions=counts[name],
            moved=moved.get(name, Decimal("0")),
            moves=moves.get(name, 0),
        )
