...
def test_a_week_is_the_iso_week_it_was_made_in():
    # The 5th of January 2026 is a Monday, so it opens week 02.
    assert periods.week_of(row(made="2026-01-05", posted="2026-01-08")) == "2026-W02"
...
def test_a_week_belongs_to_its_iso_year_not_its_calendar_year():
    ...
    the summary.
    """
    assert periods.week_of(row(made="2027-01-01")) == "2026-W53"
    assert periods.week_of(row(made="2025-12-29")) == "2026-W01"


def test_weeks_sort_in_the_order_they_happened():
    made = ["2025-12-29", "2026-01-05", "2027-01-01"]
    labels = [periods.week_of(row(made=day)) for day in made]
    assert labels == sorted(labels)


def test_an_unknown_period_is_refused_by_name():
    with pytest.raises(ValueError) as raised:
        periods.label_for("fortnight")
