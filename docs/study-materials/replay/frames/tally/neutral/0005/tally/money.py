...
signs the other way round is in rules.toml; the penny is not, because it is what
the currency is, not something to tune.
"""
...
def sign_convention(rows: list[Row], settings: Settings) -> list[Row]:
    ...
    guessed from the data: if more than the share in rules.toml is positive, this
    is one of the other kind, and every sign is flipped.
    ...
    if positive > len(rows) * settings.flip_when_positive_share_above:
        for row in rows:
