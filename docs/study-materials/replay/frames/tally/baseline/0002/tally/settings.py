...
def _categories(raw: dict[str, Any], source: Path) -> tuple[tuple[re.Pattern[str], str], ...]:
    ...
    for number, rule in enumerate(rules, start=1):
        ...
        try:
            ...
            raise RulesError(f"{source}: rule {number} ({category}) has a bad pattern: {exc}") from exc
...
def _months(raw: dict[str, Any], source: Path) -> int:
    ...
    if not isinstance(months, int) or isinstance(months, bool) or months < 1:
        raise RulesError(f"{source}: [recurring] months has to be a whole number of 1 or more")
    return months
...
def _share(raw: dict[str, Any], source: Path) -> float:
    ...
    if isinstance(share, bool) or not isinstance(share, (int, float)):
        raise RulesError(
            f"{source}: [money] flip_when_positive_share_above has to be a number between 0 and 1"
        )
    if not 0 < share <= 1:
        raise RulesError(
