...
def period_of(row: Row, period: str) -> str:
    """The label for a row, under whichever period the summary is grouped by."""
    try:
        return PERIODS[period](row)
    except KeyError:
        raise ValueError(f"no such period: {period!r}; try one of {', '.join(sorted(PERIODS))}") from None
