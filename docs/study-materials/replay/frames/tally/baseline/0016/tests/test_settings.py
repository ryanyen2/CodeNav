...
def row(description="TESCO STORES", amount="-10.00", made="2026-01-05"):
    ...
    return Row(made=made_date, posted=made_date, description=description, amount=Decimal(amount))
...
def test_the_order_in_the_file_is_the_order_they_are_tried():
    ...
    assert names.index("shell energy|british gas|octopus energy") < names.index(r"shell|bp\b|esso|texaco")
...
def test_a_different_file_gives_different_categories(tmp_path):
    ...
    # a line of code.
    mine = rules(tmp_path, MINIMAL.replace('category = "groceries"', 'category = "the big shop"'))
...
def test_the_transfer_wording_comes_from_the_file(tmp_path):
    mine = rules(tmp_path, MINIMAL.replace('words = ["transfer"]', 'words = ["moved to the pot"]'))
