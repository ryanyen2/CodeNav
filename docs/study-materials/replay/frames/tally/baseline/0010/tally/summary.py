...
class Summary:
    ...
    # The merges worth showing: rows written differently that were treated as one
    # transaction anyway. Exact repeats are not listed — there is nothing to see.
    ...
    uncategorised_label: str = "uncategorised"   # and what it calls the bucket
    ...
    def text(self) -> str:
        ...
        for period in self.periods:
            ...
            for name in sorted(period.by_category, key=lambda k: period.by_category[k]):
                # The bucket is not a category anybody chose, so it is reported in
                # the count line rather than standing in this list as though it
                # were one of the reader's own categories.
                if name == self.uncategorised_label:
                    continue
        ...
        if self.recurring:
            ...
            out.append("")
        if self.merged:
            # Shown because these are the ones that might be wrong: two rows that
            # said different things, counted once.
            out.append("## Merged")
            out.append("")
            for merge in self.merged:
                out.append(
                    f"  {merge.day}  {money.round_total(merge.dropped.amount):>10}  "
                    f"{merge.kept.description} / {merge.dropped.description}"
                )
            out.append("")
        return "\n".join(out).rstrip() + "\n"
...
def summarise(raw: str, settings: Settings, by: str = "month") -> Summary:
    ...
    `by` is more than which heading a row lands under. rules.toml answers three
    questions per summary, and this is which set of answers to use: how a period
    is labelled, which of the two dates decides it, and what makes two rows one
    transaction. The monthly and weekly summaries of one statement can therefore
    disagree about how many transactions there were, and are meant to.

    Recurring is the exception: counted in months, by the date the payment was
    made, whichever summary this is — see recurring.py for why.
    """
    # Resolved before the rows, so a bad name always says so, even on an empty file.
    label_of = periods.label_for(by, settings.period_dates["month"])
    rows: list[Row] = read(raw)
    result = Summary(rows_read=len(rows), by=by,
                     transfer_label=settings.transfer_category,
                     uncategorised_label=settings.uncategorised)
    if not rows:
        return result

    money.sign_convention(rows, settings)

    kept, merges = dedupe.drop_duplicates(rows, settings, by)
    result.duplicates = len(merges)
    result.merged = [merge for merge in merges if merge.reworded]
    ...
    # Merchants nothing matches. Gathered first and raised together, so one run
    # gives the whole list rather than the next name each time.
    unknown = [row for row, name in counted if name == settings.uncategorised]
    if unknown and settings.unmatched == "stop":
        raise categories.Unmatched(unknown, settings.source)
    result.uncategorised = len(unknown)
