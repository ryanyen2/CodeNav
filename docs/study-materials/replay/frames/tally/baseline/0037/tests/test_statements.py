...
def transfers_shown(tmp_path, how: str):
    """The rules the tool ships with, handling transfers some other way."""
    path = tmp_path / "rules.toml"
    text = settings.DEFAULT_RULES.read_text(encoding="utf-8")
    path.write_text(text.replace('show = "apart"', f'show = "{how}"'), encoding="utf-8")
...
def test_transfers_can_be_counted_as_spending_instead(tmp_path):
    ...
    result = summarise(RAW["current"], transfers_shown(tmp_path, "spending"))
    january = next(p for p in result.periods if p.name == "2026-01")
...
def test_transfers_can_be_left_out_altogether(tmp_path):
    # What the tool used to do, for anybody who wants it back.
    result = summarise(RAW["current"], transfers_shown(tmp_path, "never"))
    january = next(p for p in result.periods if p.name == "2026-01")
