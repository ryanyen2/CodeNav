...
from tally import categories, dedupe, money, months, recurring
...
def test_the_first_matching_rule_wins():
    ...
    assert categories.categorise(row("SHELL ENERGY")) == "utilities"
    assert categories.categorise(row("SHELL 4417")) == "fuel"
...
def test_anything_unmatched_goes_to_a_bucket():
    ...
    assert categories.categorise(row("MOONLIGHT RECORDS")) == "uncategorised"
...
def test_a_transaction_belongs_to_the_month_it_was_made():
    ...
    assert months.month_of(row(made="2026-01-31", posted="2026-02-02")) == "2026-01"


def test_refunds_net_against_the_category():
    assert months.net([Decimal("-30.00"), Decimal("10.00")]) == Decimal("-20.00")


# ── duplicates and transfers ─────────────────────────────────────────────────

def test_the_same_transaction_twice_is_dropped_once():
    kept, dropped = dedupe.drop_duplicates([row(), row()])
    assert len(kept) == 1 and dropped == 1


def test_two_transactions_that_differ_are_both_kept():
    kept, dropped = dedupe.drop_duplicates([row(amount="-10.00"), row(amount="-11.00")])
    assert len(kept) == 2 and dropped == 0


def test_a_transfer_is_recognised_by_its_wording():
    assert dedupe.is_transfer(row("Transfer to savings"))
    assert not dedupe.is_transfer(row("TESCO STORES"))
...
def test_a_payment_in_three_months_at_one_amount_is_recurring():
    ...
    assert "netflix.com" in recurring.find(rows)
...
def test_the_same_merchant_at_different_amounts_is_not():
    ...
    assert recurring.find(rows) == set()
...
def test_a_transfer_pair_is_not_a_duplicate():
    ...
    kept, dropped = dedupe.drop_duplicates(pair)
    ...
    kept, dropped = dedupe.drop_duplicates(ordinary)
    assert len(kept) == 1 and dropped == 1
