...
def run(name: str):
    return summarise((FIXTURES / f"{name}.csv").read_text(encoding="utf-8"))
...
def test_a_refund_nets_within_its_own_month_and_not_across_months():
    ...
    """
    result = run("current")
    january = next(m for m in result.months if m.name == "2026-01")
    february = next(m for m in result.months if m.name == "2026-02")
...
def test_spending_exported_as_positive_is_flipped():
    ...
    for month in run("other-bank").months:
...
def test_a_transaction_made_on_the_31st_is_january():
    january = next(m for m in run("boundary").months if m.name == "2026-01")
...
def test_and_the_ones_made_in_february_are_february():
    ...
    february = next(m for m in run("boundary").months if m.name == "2026-02")
...
def test_months_come_out_in_order(name):
    names = [m.name for m in run(name).months]
