...
# shipped file it under one name and carry on, which is what RULES is; STOPS is
# the same rules with the other setting, for the handful of tests at the bottom
# that are about a run stopping instead.
RULES = replace(SHIPPED, unmatched="bucket")
STOPS = replace(SHIPPED, unmatched="stop")
...
def test_a_weekly_shop_written_two_ways_is_one_shop_in_the_week():
    ...
    week = next(p for p in result.periods if p.name == "2026-W05")
...
def test_a_merchant_nothing_matches_can_stop_the_run():
    with pytest.raises(Unmatched) as raised:
        summarise(RAW["current"], STOPS)
...
def test_an_unmatched_merchant_is_filed_under_one_name():
    # What the rules ship with: the merchant is filed under a name of its own,
    # counted, and reported in the count line at the end of the summary.
    result = summarise(RAW["current"], RULES)      # unmatched = "bucket"
    assert result.uncategorised == 1
    assert "1 uncategorised" in result.line()
