...
from .categories import Unmatched
...
def main(argv: list[str] | None = None) -> int:
    ...
    # `-` means stdout and is a word here, but anything else starting with a dash
    # was meant as an option. Left alone it would be taken for a file name, and
    # `--by-month` — which somebody will type, having seen --by-week — would come
    # back as "no such file" rather than "there is no such option".
    unknown = [arg for arg in args if arg.startswith("-") and arg != "-"]
    if unknown:
    ...
    if command == "check":
        ...
        if not found:
            ...
            return 2
        # One statement stopping does not stop the others. The point of `check`
        # is to see the whole folder in one go, and a statement with an unknown
        # merchant in it is exactly what somebody is running this to find.
        stopped = 0
        for path in found:
            try:
        ...
        print(f"checked {len(found)} statements, {stopped} stopped")
