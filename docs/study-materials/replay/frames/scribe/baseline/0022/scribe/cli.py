...
def _convert_one(path: Path, out: str | None, conf: config.Config, write_report: bool) -> int:
    ...
    if out == "-":
        ...
        # The summary goes to stderr here, not stdout. It used to go to stdout
        # along with the Markdown, which meant `scribe convert x.txt - > x.md`
        # wrote the stats line into the document.
