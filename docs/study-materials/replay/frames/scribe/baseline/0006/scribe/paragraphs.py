...
def dehyphenate(
    ...
    are `keep_hyphen`, the prefixes where a hyphen usually is part of the word:
    without them "well-being" split across lines comes back as "wellbeing", and
    nothing in the text says which was meant. The alternative — keeping every
    hyphen, which is `keep_all_hyphens` — is defensible for a corpus of technical
