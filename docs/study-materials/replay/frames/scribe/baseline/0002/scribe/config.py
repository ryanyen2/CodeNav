...
def _apply(current: Any, table: dict[str, Any], where: str) -> Any:
    ...
    hints = {f.name: f.type for f in fields(current)}
    resolved = _hints(type(current))
    changes: dict[str, Any] = {}
    for key, value in table.items():
        if key not in hints:
            known = ", ".join(sorted(hints))
            raise ConfigError(f"{where}: unknown setting '{key}'. Known settings: {known}")
        changes[key] = _coerce(value, resolved[key], f"{where}.{key}")
    return replace(current, **changes)


def _hints(cls: type) -> dict[str, Any]:
    """Field types with the string annotations resolved."""
    import typing
