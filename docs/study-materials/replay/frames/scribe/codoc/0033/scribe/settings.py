...
# A starting point for `paragraphs.keep_hyphen`, and deliberately not the
# default. These are the prefixes where a hyphen is usually part of the word
# rather than something the typesetter added, so with none of them set
# "well-being" broken across a line comes back as "wellbeing".
#
# It is not the default because a list nobody chose is a list nobody can argue
# with: it is wrong for any corpus whose compounds are not these, and silently
# so. A document that wants them says so, and copying this line into a config
# file is how:
#
#     keep_hyphen = ["co", "e", "ex", "non", "post", "pre", "re", "self", "semi",
#                    "sub", "un", "well"]
SUGGESTED_KEEP_HYPHEN = (
...
class Settings:
    ...
    # ── paragraphs ───────────────────────────────────────────────────────────
    # Empty, so that every hyphen a document keeps is one it asked to keep.
    # `SUGGESTED_KEEP_HYPHEN` above is the list to start from.
...
def _read_value(kind: str, value: object, where: str) -> object:
    if kind == "int":
        ...
        return value
    if kind == "repeats":
        # One would mean a line appearing on a single page is a running header,
        # which is every line near an edge. There is no document that wants that,
        # so it is refused here rather than left to produce an empty conversion.
        if not isinstance(value, int) or isinstance(value, bool) or value < 2:
            raise ConfigError(f"{where}: expected a whole number of 2 or more, got {value!r}")
        return value
    if kind == "share":
