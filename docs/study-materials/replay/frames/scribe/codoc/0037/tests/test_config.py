...
def test_the_defaults_are_the_values_the_constants_had():
    ...
    Two have moved from what the constants held.

    `repeat_share` was 0.6, which on a five page report puts the threshold at
    three pages and leaves a header that appears on two of them in the prose. 0.4
    is two pages in five exactly, and it holds for six and seven page documents
    too, where 0.5 would not. The cost is on long documents, where a line near an
    edge on 16 pages of 40 is now furniture where it used to take 24.
    `repeat_share = 0.6` in a config file restores it.

    `keep_hyphen` was a list of twelve prefixes shipped with the program. It is
    empty, because which compounds a corpus contains is a fact about the corpus
    and the shipped list was a guess at it. scribe.toml carries the old list as
    ...
    assert DEFAULTS.paragraphs.keep_hyphen == frozenset()
...
def test_a_prefix_is_opted_into_rather_than_shipped():
    # The default keeps no hyphen at all. Naming the prefix is what keeps it, and
    # it keeps that one only.
    assert paragraphs.dehyphenate("well-", "being matters") == ("wellbeing", "matters")

    settings = config.parse('[paragraphs]\nkeep_hyphen = ["well"]').for_document("x.txt")
    assert paragraphs.dehyphenate("well-", "being matters", settings.paragraphs) == (
        "well-being", "matters")
    assert paragraphs.dehyphenate("settle-", "ment had moved", settings.paragraphs) == (
        "settlement", "had moved")


def test_a_document_can_have_its_own_prefixes():
    # The reason for opting in per document: one corpus, two kinds of writing.
    conf = config.parse(
        '[document."handbook.txt".paragraphs]\nkeep_hyphen = ["self", "non"]\n'
    )
    handbook = conf.for_document("handbook.txt").paragraphs
    memo = conf.for_document("memo.txt").paragraphs
    assert paragraphs.dehyphenate("self-", "employed people", handbook)[0] == "self-employed"
    assert paragraphs.dehyphenate("self-", "employed people", memo)[0] == "selfemployed"


def test_the_prefixes_are_matched_without_case():
    settings = config.parse('[paragraphs]\nkeep_hyphen = ["well"]').for_document("x.txt")
    assert paragraphs.dehyphenate("Well-", "being matters", settings.paragraphs)[0] == "Well-being"
...
def tag(page: int) -> str:
    """A short word unique to a page: a, b, ... z, aa, ab. No digits in it."""
    out = ""
    page += 1
    while page:
        page, letter = divmod(page - 1, 26)
        out = chr(ord("a") + letter) + out
    return out
...
def pages(*, header_at: int = 0, count: int = 3, header_on: set[int] | None = None) -> str:
    ...
    for page in range(count):
        lines = [f"{tag(page)}: {body}" for body in BODY]
