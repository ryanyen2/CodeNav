...
report is the receipt — what was thrown away, which notes moved and where to find
them, and which settings were in force — because the lossy steps leave no trace
in the Markdown itself. The notes are listed in the order the Markdown ends with,
each against the `[^n]` to search for. It
