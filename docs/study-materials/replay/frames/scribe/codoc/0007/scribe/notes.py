...
One policy lives here. What it is tuned by is in scribe/config.py, under
[notes]; the reasoning for the defaults is below, next to the rule each one
governs.
"""
...
from .config import DEFAULTS, NoteSettings

SETTINGS = DEFAULTS.notes
...
def looks_like_note(
    text: str, from_bottom: int, settings: NoteSettings = SETTINGS
) -> bool:
    """Whether a line at the foot of a page is a note rather than prose.

    Position is what separates them: a line within `foot_zone` of the bottom. The
    same shape in the middle of a page is a numbered list item, and treating
    those as notes would move half a list to the end of the document.
    """
    return from_bottom <= settings.foot_zone and bool(NOTE_LINE.match(text))
...
def mark(text: str, settings: NoteSettings = SETTINGS) -> str:
    ...
    word. The alternative — leaving them where they were, which is `collect =
    false` — keeps the page's original look and is what you would want if the
    notes were asides meant to be read in place.
    """
    if not settings.collect:
        return text
