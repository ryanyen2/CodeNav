...
def test_the_defaults_are_the_values_the_rules_used_to_hard_code():
    assert (DEFAULTS.edge, DEFAULTS.repeat_share, DEFAULTS.min_pages) == (2, 0.6, 3)
...
def test_keeping_every_hyphen_is_a_single_flag():
    ...
    assert paragraphs.dehyphenate("settle-", "ment had moved", every) == ("settle-ment", "had")
...
def test_settings_change_the_document_and_leave_nothing_behind():
    raw = "1. A heading that runs to rather more words than usual here\n\nBody.\n"
    tight = convert(raw, Settings(max_heading_words=4))
    loose = convert(raw, Settings(max_heading_words=20))
    assert tight.headings == 0 and loose.headings == 1
    # No module state, so the defaults are untouched by either call.
    assert convert(raw).headings == 0
