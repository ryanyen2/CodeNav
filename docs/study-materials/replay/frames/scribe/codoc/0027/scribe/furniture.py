...
# How near an edge a line has to be, how long a page has to be for "near an edge"
# to mean anything, and how much of the document a line has to appear on, are all
# settings now: see `scribe/settings.py`. Two pages out of three is a running
# header; two pages out of forty is a coincidence, and which of those a corpus
# looks like is exactly the sort of thing that changes per document.
...
def find_repeated(doc: Document, settings: Settings = DEFAULTS) -> set[str]:
    ...
    # The share of the document, with a floor under it in pages. On a five page
    # report the share alone asks for three, which misses a header the title page
    # and a full-page figure between them kept off two of the five.
    threshold = max(settings.min_repeats, int(len(doc.pages) * settings.repeat_share))
