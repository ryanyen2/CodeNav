...
TOPICS = [
    "tides", "wind", "access", "light", "sand", "rock", "weed", "surf", "dune", "cliff",
    "paths", "gates", "fences", "marsh", "inlets", "spits", "bays", "coves", "reefs", "shoals",
]


def page_ending_in(note: str, topic: str) -> str:
    """A page long enough to have a foot, with one note sitting at it.

    Every line carries the page's own topic, so no two pages share a line. Pages
    built from a template repeat, and a repeated line is furniture: the notes of
    an earlier draft of this were stripped off the page before anything looked
    for them, and the test was measuring the fixture.
    """
    prose = f"The survey team wrote at length about the {topic}."
    filler = [f"More about the {topic}, {part}." for part in
              ("continued", "further", "in detail", "at last")]
    return "\n".join([prose, ""] + filler + [note]) + "\n"


def notes_out_of_order():
    """Two pages whose notes are numbered the wrong way round.

    Not a contrivance: a source that restarts its numbering on every page gives
    this, and so does one where the notes were renumbered by hand. It is the case
    that tells document order apart from sorted order.
    """
    return convert(
        page_ending_in("2 The note that carries a two.", "The first page of prose.")
        + "\f"
        + page_ending_in("1 The note that carries a one.", "The second page of prose.")
    )


def test_the_notes_are_listed_in_the_order_the_document_has_them():
    result = notes_out_of_order()
    out = report.render(result, Path("odd.txt"), Path("odd.md"))

    in_markdown = re.findall(r"^\[\^(\d+)\]:", result.markdown, re.M)
    in_report = re.findall(r"^\d+\. `\[\^(\d+)\]`", out, re.M)

    assert in_markdown == ["2", "1"], "the document keeps the source's own order"
    assert in_report == in_markdown, "so the report has to as well, not sorted"


def test_the_numbering_is_the_position_not_the_marker():
    # The point of the left hand number: it counts down the list in the Markdown,
    # so it still cross-references when the markers repeat or run backwards.
    out = report.render(notes_out_of_order(), Path("odd.txt"), Path("odd.md"))
    assert re.findall(r"^(\d+)\. `\[\^", out, re.M) == ["1", "2"]


def test_a_note_is_quoted_by_its_opening_words():
    assert "The 2019 report describes the method in full." in rendered()


def test_a_long_note_is_cut_short():
    # The whole note is a line away in the Markdown. The report is for matching
    # the two up, not for holding a second copy of them.
    result = convert(page_ending_in("1 " + "a very long note indeed " * 10, "Prose."))
    out = report.render(result, Path("long.txt"), Path("long.md"))
    quoted = next(line for line in out.splitlines() if line.startswith("1. `[^1]`"))
    assert quoted.endswith("...")
    assert len(quoted) < 90


def test_a_document_of_many_notes_is_still_a_short_report():
    pages = "\f".join(
        page_ending_in(f"{n} Note number {n} says something.", f"Prose on page {n}.")
        for n in range(1, 21)
    )
    result = convert(pages)
    out = report.render(result, Path("many.txt"), Path("many.md"))
    assert result.notes == 20
