...
def find_repeated(doc: Document, settings: FurnitureSettings = SETTINGS) -> set[str]:
    ...
    # Two things decide the threshold, and the larger wins. `repeat_share` is a
    # share of the document, which is what makes "repeated" mean anything on a
    # long one: two pages out of forty is a coincidence. `min_repeats` is the
    # floor, and it is what carries short documents, where a share of a small
    # number rounds down to something meaningless.
