...
Settings come from the nearest `scribe.toml`, if there is one, and each document
gets the values its own section asks for. Without a config file every document
...
def _convert_one(path: Path, out: str | None, conf: config.Config) -> int:
    ...
    result = convert(raw, settings)
    if out == "-":
        sys.stdout.write(result.markdown)
    else:
        target = path.with_suffix(".md")
        target.write_text(result.markdown, encoding="utf-8")
        write_report(target.with_name(f"{path.stem}.conversion.md"),
                     path.name, result, settings)
