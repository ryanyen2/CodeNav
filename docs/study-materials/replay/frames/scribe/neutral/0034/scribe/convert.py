...
class Converted:
    ...
    # The notes as they were written out, in that order, so the report can be
    # read down one column against the foot of the Markdown.
    note_list: list[tuple[str, str]] = field(default_factory=list)
...
def convert(raw: str, settings: Settings = DEFAULTS) -> Converted:
    ...
    if collected.footnotes:
        # Page order, then order down the page, which is the order they are
        # written in and so the order the report lists them in.
        for number, body in collected.footnotes:
            out.append(f"[^{number}]: {body}")
            result.note_list.append((number, body))
            result.notes += 1
