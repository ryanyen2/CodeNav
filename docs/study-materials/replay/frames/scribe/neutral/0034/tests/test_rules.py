...
def test_a_real_compound_loses_its_hyphen_unless_asked_for():
    """"well-being" split across lines comes back as "wellbeing" by default.

    This used to go the other way, on a list of prefixes shipped with the
    program. The list is still available and is in scribe.toml, but it starts
    empty: which compounds a corpus contains is a fact about that corpus. Naming
    "well" in `keep_hyphen` restores this one, and test_config.py has that.
    """
