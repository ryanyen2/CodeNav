...
from scribe.settings import find

FIXTURES = Path(__file__).parent.parent / "fixtures"


def md(name: str) -> str:
    """The document converted with the built-in defaults."""
    return convert((FIXTURES / f"{name}.txt").read_text(encoding="utf-8")).markdown


def configured(name: str) -> str:
    """The document converted the way fixtures/scribe.toml asks for."""
    source = FIXTURES / f"{name}.txt"
    settings = find(source).for_document(source.name)
    return convert(source.read_text(encoding="utf-8"), settings).markdown
...
def test_footnotes_are_collected_at_the_end():
    ...
    assert out.index("[^1]: The 2019 report") > out.index("## Recommendations")
...
# ── what the config file changes ─────────────────────────────────────────────

def test_the_memo_needs_a_hyphen_the_defaults_do_not_keep():
    # "part-" / "signed" across a line break. "part" is not one of the default
    # prefixes, so out of the box the memo says "partsigned".
    assert "partsigned" in md("memo")


def test_and_its_config_block_fixes_it():
    out = configured("memo")
    assert "part-signed" in out
    assert "partsigned" not in out


def test_the_config_leaves_the_other_documents_alone():
    # The override is scoped to memo.txt. The report is justified and full of
    # hyphens the typesetter put in, and keeping them all would be wrong.
    assert configured("report") == md("report")
    assert configured("handbook") == md("handbook")
