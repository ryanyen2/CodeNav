...
Two policies live here. The list of prefixes that keep their hyphen comes from
the settings now, and the default is the list this file always had.
"""
...
def dehyphenate(
    ...
    if not tail or not tail[0].isalpha():
        ...
        return None
    stem = match.group(1)
    if stem.casefold() in settings.keep_hyphen:
