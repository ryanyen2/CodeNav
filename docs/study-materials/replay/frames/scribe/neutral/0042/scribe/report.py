...
def _excerpt(body: str, width: int = 60) -> str:
    """Enough of a note to recognise it by, cut at a word.

    The whole note is in the Markdown a line away. What is wanted here is enough
    to match the two up without the report turning into a second copy of them.
    """
    if len(body) <= width:
        return body
    return body[:width].rsplit(" ", 1)[0] + "..."
...
def render(result: Converted, source: Path, markdown: Path) -> str:
    ...
    if result.collected_notes:
        out += [
            "",
            "## Footnotes moved to the end",
            "",
            "Taken from the foot of the page they sat on and collected at the end of "
            "the Markdown, in this order. The number on the left is the position in "
            "that list; the marker beside it is the label the note carries, which is "
            "the source's own numbering and is not always the same:",
            "",
        ]
        for position, (number, body) in enumerate(result.collected_notes[:LIMIT], start=1):
            out.append(f"{position}. `[^{number}]` {_excerpt(body)}")
        if len(result.collected_notes) > LIMIT:
            out.append(f"{LIMIT + 1}. and {len(result.collected_notes) - LIMIT} more")
