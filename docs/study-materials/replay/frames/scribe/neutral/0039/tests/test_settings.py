...
def test_the_hyphen_list_starts_empty():
    """Opt in, not opt out.

    The twelve prefixes are still written down, because the knowledge is worth
    keeping, but a document that wants them has to say so. A list nobody chose is
    a list nobody can argue with, and it is wrong for any corpus whose compounds
    are not these.
    """
    assert DEFAULTS.keep_hyphen == frozenset()
    assert "well" in SUGGESTED_KEEP_HYPHEN


def test_the_suggested_list_is_a_config_file_away():
    # The line in the comment above SUGGESTED_KEEP_HYPHEN has to actually work.
    settings = Settings(keep_hyphen=frozenset(SUGGESTED_KEEP_HYPHEN))
    assert paragraphs.dehyphenate("well-", "being matters", settings) == ("well-being", "matters")
    assert paragraphs.dehyphenate("settle-", "ment moved", settings) == ("settlement", "moved")
