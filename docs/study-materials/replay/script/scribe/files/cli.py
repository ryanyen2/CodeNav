...
Settings come from the nearest `scribe.toml`, if there is one. Without a config
file every document converts exactly as it did before.
"""
...
from . import config, furniture
from .convert import convert
from .report import write_report


def _apply(settings) -> None:
    """Put the settings where the rules read them."""
    furniture.REPEAT_SHARE = settings.repeat_share
    furniture.EDGE = settings.edge


def _convert_one(path: Path, out: str | None, conf: config.Config) -> int:
    raw = path.read_text(encoding="utf-8", errors="replace")
    settings = conf.for_document(path.name)
    result = convert(raw)
    if out == "-":
        ...
        write_report(target.with_name("report.md"), path.name, result, settings)
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        conf = config.load(config.find(Path.cwd()))
    except config.ConfigError as err:
        print(f"scribe.toml: {err}", file=sys.stderr)
        return 2
    _apply(conf.defaults)
    ...
    if command == "convert":
        ...
        if not path.is_file():
            ...
            return 2
        return _convert_one(path, rest[1] if len(rest) > 1 else None, conf)
