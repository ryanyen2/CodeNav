...
| From `CLAUDE.md`, written by `/init` | 10/12 | 9/12 |
| From the codoc tree, with the orientation pass | 10/12 | 9/12 |

The two arms now score the same, and that is the honest state. An earlier draft
had codoc a point ahead, and rewriting the questions toward coupling and
consequence — what breaks, what a choice cost — took the lead away, because
`/init` writes a good section on exactly that. If codoc is to come out ahead it
has to be because it carried something better, not because the quiz was aimed.

Both descriptions are now generated by their own tool: `CLAUDE.md` by Claude
Code's `/init`, the tree by `codoc init`. Neither is hand-written, which is the
...
**Q1. Somebody moves the character normalising so it runs first instead of last. What breaks?**
...
**Q2. Somebody makes heading detection stricter so fewer things become headings. Which other rule is most likely to start behaving differently?**
...
- c) **Paragraph reflow, because what is no longer a heading joins the prose around it** ✓
- d) None: heading detection is decided line by line and touches nothing else

**Q3. Which piece of information does scribe have while removing page furniture, and no longer have afterwards?**
- a) Which words were hyphenated across a line break
- b) The order the pages came in
- c) **Where each line sat on its page** ✓
- d) Which characters were ligatures

**Q4. The output cannot be both faithful to the page and easy to search. Which did scribe choose?**
- a) **Easy to search, even where that loses how the page looked** ✓
- b) Faithful to the page, even where that makes it awkward to search
- c) Whatever the input already was, unchanged
- d) Faithful for prose and searchable for everything else
...
**Q11. Two guards stop furniture removal from firing on a short document. One is a minimum number of pages. What is the other?**
- a) A minimum number of words in the repeated line
- b) **A minimum number of lines on the page, so that a margin means anything** ✓
...
**Q12. You want the running header kept on a one-off letter but still dropped from a report. What is the obstacle?**
- a) By the time anything could tell the two apart, the header has already been removed
- b) **Nothing scribe can see distinguishes them: it is handed text and no more** ✓
