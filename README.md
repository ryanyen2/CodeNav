...
| **Slash commands** | `.claude/commands/codoc/` | `/codoc:plan <task>` — propose plan nodes *before* writing code; `/codoc:realize` — implement the directives Loop B queued. |
...
| **Hooks** | `.claude/settings.json` | `SessionStart` · `Stop` · `PreToolUse` · `PostToolUse` · `UserPromptSubmit` — maintain `.codoc/activity.json` (the live agent touch-log → VS Code gutter decorations), run recovery reflection when the session stops, and nudge `/codoc:realize` when work is queued. Fire-and-forget; they never block the agent. |
...
5. Your session runs **`/codoc:realize`** — reads the directive, writes the code,
...
.codoc/
  ...
  realize.md          — the realization queue: directives the live session implements via /codoc:realize
...
(status `awaiting_impl`). The session implements via `/codoc:realize`; the
...
Code fixtures: `test/draco/` (small Python), `test/requests/` (real-world Python),
